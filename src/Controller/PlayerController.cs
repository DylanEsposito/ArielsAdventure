...
namespace Ariel.Controllers
{
    public class PlayerController : BaseController
    {
        ...
        void Start()
        {
            ...
            stateMachine.SwitchState(PlayerStateType.Idle, this.gameObject, this.theRigidbody, this.theAnimator, this.playerConfig, this.playerInfo);
        }
        ...
        void Update()
        {
            if (!isAlive)
            {
                if (stateMachine.GetCurrentState().GetStateType() != PlayerStateType.NotAlive)
                {
                    ReactivateFootColliders();
                    stateMachine.SwitchState(PlayerStateType.NotAlive, this.gameObject, this.theRigidbody, this.theAnimator, this.playerConfig, this.playerInfo);
                }
                return;
            }
            ...
            if (CheckGround())
            {
                if (stateMachine.GetCurrentState().GetStateType() == PlayerStateType.Jumping && inAirBuffer > 0.05)
                {
                    inAirBuffer -= Time.deltaTime;
                    return;
                }
                else if (stateMachine.GetCurrentState().GetStateType() == PlayerStateType.Climbing)
                {
                    continousClimbCheck = false;
                }
                else
                {
                    if (stateMachine.GetCurrentState().GetStateType() == PlayerStateType.Jumping
                        || stateMachine.GetCurrentState().GetStateType() == PlayerStateType.WallJumping
                        || stateMachine.GetCurrentState().GetStateType() == PlayerStateType.Falling)
                    {
                        SwitchCheckIfMoving();
                        CreateDust();
                    }
        ...
        private void FixedUpdate()
        {
            ...
            if (!isGrounded && stateMachine.GetCurrentState().GetStateType() != PlayerStateType.Dashing
                && stateMachine.GetCurrentState().GetStateType() != PlayerStateType.WaterDashing
        ...
        private void SwitchCheckIfMoving()
        {
            ...
            if (isGrounded)
            {
                ...
                {
                    stateMachine.SwitchState(PlayerStateType.Running, this.gameObject, this.theRigidbody, this.theAnimator, this.playerConfig, this.playerInfo);
                }
                else
                {
                    stateMachine.SwitchState(PlayerStateType.Idle, this.gameObject, this.theRigidbody, this.theAnimator, this.playerConfig, this.playerInfo);
            ...
            {
                stateMachine.SwitchState(PlayerStateType.Falling, this.gameObject, this.theRigidbody, this.theAnimator, this.playerConfig, this.playerInfo);
            }
        }
        ...
        private void SwitchWaterState()
        {
            switch (stateMachine.GetCurrentState().GetStateType())
            {
                case PlayerStateType.WaterDashing:
                    break;
                default:
                    stateMachine.SwitchState(PlayerStateType.Swimming, this.gameObject, this.theRigidbody, this.theAnimator, this.playerConfig, this.playerInfo);
                    break;
            }
        }
        ...
        public void OnJump(InputAction.CallbackContext context)
        {
            ...
            if (context.started && coyoteTimeCounter > 0f && !isWallSliding)
            {
                ...
                CreateDust();
                ReactivateFootColliders();
                stateMachine.SwitchState(PlayerStateType.Jumping, this.gameObject, this.theRigidbody, this.theAnimator, this.playerConfig, this.playerInfo);
            }
            else if (context.canceled && !playerInfo.isContextCanceled() && stateMachine.GetCurrentState().GetStateType() == PlayerStateType.Jumping && jumpingBuffer >= 0)
            {
            ...
            if (context.started && (playerInfo.climbLeft || playerInfo.climbRight || isWallSliding))
            {
                ...
                wallCheckTimer = wallCheckCooldown;
                ReactivateFootColliders();
                stateMachine.SwitchState(PlayerStateType.WallJumping, this.gameObject, this.theRigidbody, this.theAnimator, this.playerConfig, this.playerInfo);
            }
        }
        ...
        public void OnClimb(InputAction.CallbackContext context)
        {
            ...
            //DEE test
            if (context.started && stateMachine.GetCurrentState().GetStateType() != PlayerStateType.Climbing)
            {
        ...
        {
            if (stateMachine.GetCurrentState().GetStateType() == PlayerStateType.Climbing)
            {
        ...
        public IEnumerator PhaseIntoClimb(bool pClimbLeft, bool pClimbRight)
        {
            ...
            continousClimbCheck = false;
            stateMachine.SwitchState(PlayerStateType.Climbing, this.gameObject, this.theRigidbody, this.theAnimator, this.playerConfig, this.playerInfo);
        ...
        {
            if (isGrounded || stateMachine.GetCurrentState().GetStateType() == PlayerStateType.Swimming
                || stateMachine.GetCurrentState().GetStateType() == PlayerStateType.Climbing
                || stateMachine.GetCurrentState().GetStateType() == PlayerStateType.Dashing)
            {
        ...
        void WallSlide()
        {
            ...
            if (IsWalled())
            {
                if (wallCheckTimer <= 0)
                {
                    isWallSliding = true;
                    ReactivateFootColliders();
                    stateMachine.SwitchState(PlayerStateType.WallSliding, this.gameObject, this.theRigidbody, this.theAnimator, this.playerConfig, this.playerInfo);
                }
            }
            else
            {
                //Debug.Log("Is wall left hit? " + isWallLeft + " Is wall right hit? " + isWallRight);
                if (stateMachine.GetCurrentState().GetStateType() == PlayerStateType.WallSliding)
                {
                    wallCheckTimer = wallCheckCooldown;
                    ResetCoyoteTimer();
                    ReactivateFootColliders();
        ...
        private IEnumerator EightWayDash()
        {
            ...
            ReactivateFootColliders();
            stateMachine.SwitchState(PlayerStateType.Dashing, this.gameObject, this.theRigidbody, this.theAnimator, this.playerConfig, this.playerInfo);
        ...
        private IEnumerator WaterDash()
        {
            ...
            playerInfo.SetCanDash(false);
            ReactivateFootColliders();
            stateMachine.SwitchState(PlayerStateType.WaterDashing, this.gameObject, this.theRigidbody, this.theAnimator, this.playerConfig, this.playerInfo);
        ...
        public void OnMeow(InputAction.CallbackContext context)
        {
            ...
            if (context.started)
            {
                ...
                if (stateMachine.GetCurrentState().GetStateType() == PlayerStateType.Idle)
                {
        ...
        {
            if (stateMachine.GetCurrentState().GetStateType() == PlayerStateType.Swimming)
            {
                return;
            }
        ...
        private void OnTriggerExit2D(Collider2D collision)
        {
            ...
            {
                if (stateMachine.GetCurrentState().GetStateType() != PlayerStateType.WaterDashing)
                {
